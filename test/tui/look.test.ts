...
describe("TUI LOOK (off-run inspection)", () => {
    test("Alt-p templates a LOOK at the prior op's real target", async (t) => {
        ...
        try {
            ...
            tui.write("### EDIT_ (worker:///plan.md)\nlook-probe-42\r");
            ...
            tui.write("\x1bp");
            await tui.waitFor(/### LOOK_ \(worker:\/\/\/plan\.md\)/);
    ...
    test("submitting LOOK harvests and renders the entry content off-run", async (t) => {
        ...
        try {
            await tui.waitFor(/plurnk.*\/help/);
            tui.write("### EDIT_ (worker:///note.md)\nlook-harvest-99\r");
            ...
            tui.write("### LOOK_ (worker:///note.md)\r");
            await tui.waitFor(/### LOOK_ \(worker:\/\/\/note\.md\)[\s\S]*look-harvest-99/);
            tui.write("/quit\r");
            assert.equal(await tui.exited, 0);
        } finally {
            tui.kill();
