...
2. Print banner; enter readline loop with the `  : ` prompt (2-space indent, bold ASCII colon; flips to `? ` when an ask-default toggle exists). **Emoji are banned from the prompt** — settled empirically: readline re-renders prompt+line on history-nav/backspace/completion and repositions the cursor at its own computed width; emoji cell-width disagreement between node and the terminal shifts the text after the colon on every refresh. The glyphful pre-rendered-row experiment (`  👤 ✉️  ✅ 201 : `) is dead for this reason. Output lines keep their glyphs freely — no cursor positioning happens on output.
