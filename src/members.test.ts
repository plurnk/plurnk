...
test("[§cli-file-members] list renders every definition with what its glob resolved to", async () => {
    ...
    assert.deepEqual(h.calls, [{ method: "workspace.members.list", params: {} }]);
    const text = h.out.join("");
    assert.match(text, /docs\s+service\s+active\s+include docs\/\*\* → 12 files \(3 ignored\)\n/);
    assert.match(text, /no-tokenizer\s+workspace\s+active\s+exclude \*\*\/tokenizer\.json → 4 members\n/);
    assert.match(text, /note\s+workspace\s+active\s+include note\.md → 1 file\n/);
    assert.match(text, /drafts\s+workspace\s+disabled\s+include drafts\/\*\.md\n/);
    assert.match(text, /bad\s+workspace\s+unavailable\s+include \.\.\/x\/\*\*\s+— outside the project root/);

    const explicit = harness({ "workspace.members.list": { definitions: [] } });
    await handleMembers("", explicit.rpc, explicit.write);
    await handleMembers("", explicit.rpc, explicit.write);
    assert.deepEqual(explicit.calls, [{ method: "workspace.members.list", params: {} }, { method: "workspace.members.list", params: {} }]);
...
test("[§cli-file-members] discover sends the path or glob verbatim and renders the daemon's verdict", async () => {
    ...
    assert.deepEqual(h.calls, [
        { method: "workspace.members.discover", params: { query: "README.md" } },
        { method: "workspace.members.discover", params: { query: "docs/**" } },
        { method: "workspace.members.discover", params: { query: "!**/tokenizer.json" } },
    ]);
...
test("[§cli-file-members] add composes one exact { glob } definition; enable, disable, and remove map to workspace actions", async () => {
    const h = harness({
    ...
    assert.deepEqual(h.calls, [
        { method: "workspace.members.add", params: { alias: "docs", definition: { glob: "docs/**" } } },
        { method: "workspace.members.add", params: { alias: "no-tokenizer", definition: { glob: "!**/tokenizer.json" } } },
        { method: "workspace.members.enable", params: { alias: "docs" } },
        { method: "workspace.members.disable", params: { alias: "docs" } },
        { method: "workspace.members.remove", params: { alias: "docs" } },
    ]);
    const text = h.out.join("");
...
test("[§cli-file-members] an unavailable outcome renders the daemon's Problem beside the state", async () => {
    const h = harness({
        "workspace.members.add": { status: 201, alias: "ghost", definition: { alias: "ghost", state: "unavailable", problem: { detail: "the workspace has no project root" } } },
    });
