...
2. Print the banner; enter the readline loop with one fixed four-cell slot followed by the `: ` input affordance. The slot holds a centered, stable-wide 🔥 while YOLO is armed (blank otherwise); while embedding, search, or branch work reports progress, its right-aligned ASCII percentage replaces the badge. The colon therefore never moves. Identity and lifecycle state belong to waterfall rows and diagnostics, not the prompt. **Prompt glyph policy: no VS16/width-ambiguous sequences, ever**; output lines may render them because output requires no cursor positioning.
