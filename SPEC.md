...
**Coordinate on every line.** Each waterfall line opens with the `LL/TT/SS` logical coordinate (loop/turn/sequence, zero-padded min-2), so any line is its own `log://` address. Sources: log entries carry it on the wire (§13, loop_seq/turn_seq/sequence); stream lines take it from the exec URI's trailing `/L/T/S` (`exec://sh/1/1/1` → `01/01/01`); the readline prompt shows the coordinate the typed line WILL get — the next loop's foist EDIT at `<next>/01/01`, advancing as loops complete. No line is coordinate-less.
