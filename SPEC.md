...
2. Print the banner; enter the readline loop with the `[🔥][ progress%]: ` prompt. The stable-wide 🔥 occupies the two-cell mode gutter while YOLO is armed (blank otherwise); the optional ASCII percentage reports active embedding, search, or branch progress; `:` is the input affordance. Identity and lifecycle state belong to waterfall rows and diagnostics, not the prompt. **Prompt glyph policy: no VS16/width-ambiguous sequences, ever**; output lines may render them because output requires no cursor positioning.
