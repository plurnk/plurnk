...
test("{§worker-model-selection}: TUI admission fails when durable model truth cannot be read", async () => {
    ...
    const transport: Transport = {
        ...
        observe: () => { throw new Error("the TUI observed a model after failed admission"); },
...
test("lookStatement: preserves the exact fence", () => {
...
test("lookStatement: preserves target, scope, and body", () => {
    assert.equal(lookStatement("```LOOK (a.ts) <1,40>```"), "```LOOK (a.ts) <1,40>```");
    assert.equal(lookStatement("```LOOK (users.json)\n$.name\n```"), "```LOOK (users.json)\n$.name\n```");
});

test("lookStatement: keeps longer fence widths without a second case grammar", () => {
    assert.equal(lookStatement("````LOOK (a.md)````"), "````LOOK (a.md)````");
    assert.equal(lookStatement("```look (a.md)```"), null);
});

test("lookStatement: rejects names other than the reserved LOOK operation", () => {
    assert.equal(lookStatement("```READ (a.md)```"), null);
    assert.equal(lookStatement("```EDIT (a.md)\nx\n```"), null);
...
test("[§cli-workers-topology] handleVerb /enter, /older, /newer, /parent hop the tree and report the path", async () => {
    ...
    assert.equal(ctx.out.at(-1), "  worker: guesser1 [/sess/~guesser1] (1/2)\n");
    assert.ok(ctx.calls.some((c) => c.method === "worker.model.get"), "policy is re-read for the newly bound worker");
    await handleVerb("/older", ctx);
    assert.deepEqual(ctx.attached, ["guesser1", "sess-fork"], "j walks to the older sibling");
    assert.equal(ctx.out.at(-1), "  worker: sess-fork [/sess/~sess-fork] (2/2)\n");
    await handleVerb("/older", ctx);
    ...
    assert.equal(ctx.out.at(-1), "  worker: sess [/~sess]\n", "a lone root has no sibling position");
...
test("handleVerb /share with no folder → usage, no rpc", async () => {
    const ctx = makeCtx();
    await handleVerb("/share", ctx);
    assert.deepEqual(ctx.calls, []);
    assert.match(ctx.out.join(""), /usage: \/share <folder>/u);
});
...
test("handleVerb /script <path> → reads the file, ships its DSL to op.parse, summarizes", async (t) => {
    ...
    await writeFile(file, "```EDIT (file://a.md)\nhi\n```\n```READ (file://a.md)```");
