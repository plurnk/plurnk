...
test("help is a compact grouped index over commands and interaction grammar", () => {
    ...
    assert.match(TUI_HELP, /language\s+## PLAN_/);
...
test("lookStatement: preserves the exact heading", () => {
    assert.equal(lookStatement("### LOOK_ (worker:///plan.md)"), "### LOOK_ (worker:///plan.md)");
    assert.equal(lookStatement("### LOOK_ (log:///1/2/3)"), "### LOOK_ (log:///1/2/3)");
});

test("lookStatement: preserves signal, target, scope, and body", () => {
    assert.equal(lookStatement("### LOOK_ [2] (a.ts) <1,40>"), "### LOOK_ [2] (a.ts) <1,40>");
    assert.equal(lookStatement("### LOOK_ (users.json)\n$.name"), "### LOOK_ (users.json)\n$.name");
});
...
test("lookStatement: rejects non-LOOK operations; trailing word characters are a legal suffix", () => {
    assert.equal(lookStatement("### READ_ (a.md)"), null);
    assert.equal(lookStatement("### EDIT_ (a.md)\nx"), null);
...
test("handleVerb /script <path> → reads the file, ships its DSL to op.parse, summarizes", async (t) => {
    ...
    await writeFile(file, "### EDIT_ (file://a.md)\nhi\n\n### READ_ (file://a.md)\n");
    ...
    assert.match((parse!.params as { text: string }).text, /### EDIT_ \(file:\/\/a\.md\)/);   // raw file text, unparsed by the client
...
test("handleVerb /script surfaces the worst op status", async (t) => {
    ...
    await writeFile(file, "### READ_ (file://gone.md)\n");
