...
2. Print banner; enter readline loop with the `  👤 ✉️  ✅ 201 : ` prompt — the prompt IS the full user-speech row, pre-rendered, so the echoed line column-aligns with the waterfall by construction (no echo erasure, no post-hoc rendering). The `201` is deliberate optimistic rendering of a constant: the prompt row is always a 201 EDIT by contract. The `: ` rhymes with nvim's cmdline and flips to `? ` when an ask-default toggle exists. Known tradeoff: ✉️ (VS16) cell-counts as 1 on a minority of terminals while readline counts 2, drifting the cursor a column during mid-line editing there; the echoed line renders correctly and forward typing is unaffected.
