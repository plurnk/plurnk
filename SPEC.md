...
3. Each line entered is dispatched:
    ...
    - Named executable backtick fences → `op.parse`; a LOOK fence is inspection (§3.1.3), never a run. Tab-completion of an operation name emits at least the canonical four-backtick fence ({§operation-fences}); submitted three-backtick operations pass through unchanged. Native OPs and executor/MCP names share this entry point; the daemon owns parsing, resolution, and diagnostics. Prefix `: ` to force prompt treatment for a literal fenced example.
